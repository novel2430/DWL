...
#define VISIBLEON(C, M)                                                        \
  ((M) && (C)->mon == (M) &&                                                   \
   (((C)->tags & (M)->tagset[(M)->seltags]) || C->issticky))
...
void buttonpress(struct wl_listener *listener, void *data) {
  ...
  case WL_POINTER_BUTTON_STATE_PRESSED:
    ...
    if (l && l->layer_surface &&
        ...
        l->scene->node.data == (void *)LyrIMPopup)
      return;
...
void setup(void) {
  ...
  char cursorsizestr[16];
  snprintf(cursorsizestr, sizeof(cursorsizestr), "%u", cursor_size);
...
void toggledimming(const Arg *arg) {
  Client *c;
  DIMOPT ^= 1;
  wl_list_for_each(c, &clients, link) { client_set_dimmer_state(c, 1); }
  c = focustop(selmon);
  if (c)
    client_set_dimmer_state(c, 0);
}

void toggledimmingclient(const Arg *arg) {
  Client *sel = focustop(selmon);
  if (sel)
    sel->neverdim ^= 1;
}
...
void togglefloating(const Arg *arg) {
  ...
  if (sel && !sel->isfullscreen) {
    ...
    if (!sel->isfloating) {
      ...
      resize(
          sel,
          (struct wlr_box){.x = newx, .y = newy, .width = neww, .height = newh},
          1);
    }
...
void remembercycletag(const Arg *arg) {
  if (arg->i == NULL)
    return;
  unsigned int current_tag = selmon->pertag->curtag - 1;
  unsigned int next_tag = -1;
...
void cycleclients_global(const Arg *arg) {
  if (!selmon)
    return;
  ...
  unsigned int curmask = (selmon->pertag && selmon->pertag->curtag >= 1 &&
  ...
  if (dir > 0) {
    if (sel) {
      Client *c;
      for (c = wl_container_of(sel->link.next, sel, link); &c->link != &clients;
    ...
    if (sel) {
      Client *c;
      for (c = wl_container_of(sel->link.prev, sel, link); &c->link != &clients;
      ...
      Client *c;
      for (c = wl_container_of(clients.prev, sel, link); &c->link != &clients;
           c = wl_container_of(c->link.prev, c, link)) {
        if (c->mon == selmon && (((c->tags & curmask) != 0) || c->issticky)) {
          focusclient(c, 1);
          arrange(selmon);
  ...
  unsigned int curidx = (selmon->pertag && selmon->pertag->curtag >= 1)
                            ? (selmon->pertag->curtag - 1)
                            : 0;

  for (int step = 0; step < TAGCOUNT; ++step) {
    curidx = (dir > 0) ? ((curidx + 1) % TAGCOUNT)
